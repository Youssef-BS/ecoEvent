...
<div class="card bg-base-100 shadow-md rounded-2xl p-4 mb-4">
    <div class="flex items-start space-x-3">
        ...
        <div class="flex-1 min-w-0">
            ...
            <!-- Post Text -->
            <p class="mt-2 text-base-content/80">{{ $post->content }}</p>

            <!-- Post Media (Image/Video) -->
            @if($post->media_url)
                @php $ext = strtolower(pathinfo($post->media_url, PATHINFO_EXTENSION)) @endphp
                @if(in_array($ext, ['jpg','jpeg','png','gif','webp']))
                <img src="{{ Storage::url($post->media_url) }}" class="rounded-lg shadow object-cover mt-3" style="width: 500px; height: 300px;">
                @elseif(in_array($ext, ['mp4','webm','ogg']))
                    <video controls class="rounded-lg shadow max-h-96 w-full mt-3">
                    ...
                    <div class="comment comment-{{ $post->id }} initial-comment" id="comment-{{ $comment->id }}">
                        <div class="flex items-start gap-2 bg-base-200 rounded-lg p-2">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm mt-1" id="comment-content-{{ $comment->id }}">{{ $comment->content }}</p>
